...
function TopCalc(queryResults, candidate) {
  ...
  for (const item of queryResults) {
    if (
      ...
      if (
        ...
        candidate.get("candidate") + candidate.get("party")
      ) {
        let partyVote = +candidate.get("votes");
        candPctRet = partyVote
      }
    }
    else if (candidate.get("candidate").includes("CANVASS")) {
      let partyVote = +candidate.get("votes");
      candPctRet = partyVote
    } else if (candidate.get("candidate").includes("TOTAL BALLOT")) {
      let partyVote = +candidate.get("votes");
      candPctRet = partyVote
    } else if (candidate.get("candidate").includes("-VOID")) {
      let partyVote = +candidate.get("votes");
      candPctRet = partyVote
    }
  }
